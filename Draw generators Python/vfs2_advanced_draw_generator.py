...
def generate_draws_bb():
    ...
    while len(draws) < 2:
        total_cost = random.choice([4, 5])  # Случайная сумма стоимости (4 или 5)
        draw = []
        current_cost = 0
        ...
        if len([el for el in draw if el in bb_figures]) <= 4:
            draws.append(draw)
...
def generate_draws_hu():
    ...
    hd_figures = ['HD-A', 'HD-C', 'HD-E', 'HD-F', 'HD-G', 'HD-J', 'HD-K']  # Массив с фигурами HD

    all_elements = hu_figures + hu_blocks + hd_figures  # Объединяем массивы
    draws = []  # Массив для хранения жеребьевок
    ...
    while len(draws) < 3:
        ...
        # Проверяем, достигли ли мы нужной суммы стоимости
        if len([el for el in draw if (el in hu_figures) and (el in hd_figures)]) <= 4:
            draws.append(draw)
            # Удаляем использованные элементы из общего списка
            all_elements = [el for el in all_elements if el not in used_elements]
