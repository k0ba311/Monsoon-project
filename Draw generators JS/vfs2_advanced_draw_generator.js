function generateDrawsBB() {
    ...
    while (draws.length < 2) {
        ...
        if ((draw.filter(el => bb_figures.includes(el)).length) <= 4) {
            draws.push(draw)
...
function generateDrawsHU() {
    ...
    const hd_figures = ['HD-A', 'HD-C', 'HD-E', 'HD-F', 'HD-G', 'HD-J', 'HD-K']; // Массив с фигурами HD

    let allElements = [...hu_figures, ...hu_blocks, ...hd_figures]; // Объединяем массивы
    let draws = []; // Массив для хранения жеребьевок
    ...
    while (draws.length < 3) {
        ...
        // Проверяем, достигли ли мы нужной суммы стоимости
        if (draw.filter(el => hu_figures.includes(el) && hd_figures.includes(el)).length <= 4) {
            draws.push(draw);
            // Удаляем использованные элементы из общего списка
            allElements = allElements.filter(el => !usedElements.includes(el));
        }
