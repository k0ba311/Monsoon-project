function generateDrawsBB() {
    ...
    let allElements = bb_figures.concat(bb_blocks); // Объединяем массивы
...
function generateDrawsHU() {
    ...
    const hd_figures = ['HD-A', 'HD-B', 'HD-C', 'HD-D', 'HD-E', 'HD-F', 'HD-G', 'HD-H', 'HD-J', 'HD-K']; // Массив с фигурами HD
    const hd_blocks = ['HD-01', 'HD-02', 'HD-03', 'HD-04', 'HD-05', 'HD-06']; // Массив с блоками HD

    let allElements = hu_figures.concat(hu_blocks, hd_figures, hd_blocks); // Объединяем массивы
    let draws = []; // Массив для хранения жеребьевок

    while (draws.length < 4) {
        let totalCost = Math.random() < 0.5 ? 5 : 6; // Случайная сумма стоимости (5 или 6)
        let draw = [];
        let currentCost = 0;
        let usedElements = []; // Для отслеживания уже использованных элементов
